...
<input type="hidden" id="au-csrf" value="{{ csrf_token() }}">
    ...
    <aside class="au-left" aria-hidden="true">
        ...
        <div class="au-left-inner">
            ...
            <div class="au-headline-block">
                ...
                <p class="au-left-desc">Become a member and unlock a world of exclusive luxury — curated products, special pricing, and a seamless experience.</p>
    ...
    <main class="au-right">
        <div class="au-card">
            ...
            <form action="{{ route('register.new') }}" method="post">
                ...
                <div class="au-otp-wrap">
                    <div class="au-otp-row">
                        <div class="au-field">
                            <label class="au-label" for="rg-otp">OTP Code <span class="au-req">*</span></label>
                            <input type="number" id="rg-otp" name="otp"
                                class="au-input @error('otp') is-invalid @enderror"
                                placeholder="Enter OTP from SMS" required />
                            @error('otp')
                                <p class="au-field-error"><i class="fas fa-times-circle"></i> {{ $message }}</p>
                            @enderror
                        </div>
                        <button type="button" id="rg-otp-send" class="au-otp-btn">
                            <i class="fas fa-paper-plane"></i> Send OTP
                        </button>
                    </div>
                    <p id="rg-otp-msg" style="display:none" class="au-otp-msg"></p>
                </div>
                @endif
...
(function () {
    /* Eye toggles */
    function eye(bId, iId, cId) {
        var b=document.getElementById(bId), i=document.getElementById(iId), c=document.getElementById(cId);
        if(!b) return;
        b.addEventListener('click', function(){
            var s=i.type==='password'; i.type=s?'text':'password';
            c.classList.toggle('fa-eye',!s); c.classList.toggle('fa-eye-slash',s);
        });
    }
    ...
    /* Strength */
    var pi=document.getElementById('rg-pass'), sw=document.getElementById('rg-strength'),
        sf=document.getElementById('rg-sf'),  sl=document.getElementById('rg-sl');
    if(pi) pi.addEventListener('input', function(){
        var v=this.value; if(!v){sw.style.display='none';return;} sw.style.display='block';
        var s=[v.length>=8,/[A-Z]/.test(v),/[0-9]/.test(v),/[^A-Za-z0-9]/.test(v)].filter(Boolean).length;
        var m=[
            {p:'25%',bg:'#e53e3e',l:'Weak',c:'#c53030'},{p:'50%',bg:'#dd6b20',l:'Fair',c:'#c05621'},
            {p:'75%',bg:'#d69e2e',l:'Good',c:'#b7791f'},{p:'100%',bg:'#38a169',l:'Strong',c:'#276749'}
        ][s-1]||{p:'25%',bg:'#e53e3e',l:'Weak',c:'#c53030'};
        sf.style.width=m.p; sf.style.background=m.bg; sl.textContent=m.l; sl.style.color=m.c;
    });

    /* Match */
    var ci=document.getElementById('rg-confirm'), mm=document.getElementById('rg-match');
    function chk(){
        if(!ci||!mm||!ci.value){if(mm)mm.style.display='none';return;}
        var ok=pi&&pi.value===ci.value;
        mm.style.display='flex'; mm.className='au-match '+(ok?'au-match-ok':'au-match-err');
        mm.innerHTML=ok?'<i class="fas fa-check-circle"></i>&nbsp;Passwords match':'<i class="fas fa-times-circle"></i>&nbsp;Passwords do not match';
    }
    if(ci){ci.addEventListener('input',chk);if(pi)pi.addEventListener('input',chk);}

    /* OTP send */
    var otpBtn=document.getElementById('rg-otp-send');
    if(otpBtn) otpBtn.addEventListener('click', function(){
        var ph=document.getElementById('rg-phone'), csrf=document.getElementById('au-csrf'), msg=document.getElementById('rg-otp-msg');
        if(!ph||!ph.value){
            if(msg){msg.style.display='flex';msg.className='au-otp-msg au-otp-err';msg.innerHTML='<i class="fas fa-exclamation-circle"></i>&nbsp;Please enter your phone number first.';}
            return;
        }
        ...
        fetch('/register/send-otp',{
            method:'POST',
            headers:{'Content-Type':'application/json','X-CSRF-TOKEN':csrf?csrf.value:'','Accept':'application/json'},
            body:JSON.stringify({number:ph.value})
        }).then(function(r){return r.json();}).then(function(){
            if(msg){msg.style.display='flex';msg.className='au-otp-msg au-otp-ok';msg.innerHTML='<i class="fas fa-check-circle"></i>&nbsp;OTP sent to your phone.';}
        }).catch(function(){
            if(msg){msg.style.display='flex';msg.className='au-otp-msg au-otp-err';msg.innerHTML='<i class="fas fa-times-circle"></i>&nbsp;Failed. Please try again.';}
        }).finally(function(){
            self.disabled=false; self.innerHTML='<i class="fas fa-paper-plane"></i>&nbsp;Resend OTP';
        });
    });
})();
</script>

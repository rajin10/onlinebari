    ...
    <div class="row">
        ...
        <div class="products col-md-9">
            ...
            <div class="container">
                ...
                <div class="row " id="grid-view"></div>
                <div class="row " id="list-view" style="display: none;"></div>
            </div>
        </div>
    </div>
</div>
...
@push('js')
   ...
   load_more(page);
    ...
    function load_more(page) {
        ...
        // page 1 => skip 0, page 2 => skip 12, etc.
        var skip = (page - 1) * 12;
        ...
        $.ajax({
            ...
            success: function(response) {
                $('.ajax-loading').hide();
                $("#grid-view").append(response.grid);
                $("#list-view").append(response.list);
                
                // console.log("Loaded products for page " + page, response);
                // console.log(response.count)
    
                if(response.grid == '' && response.list == '' && response.count == 0) {
                    // Products found, do nothing or enable 'Load More' button
                } else {
                    setTimeout(function() {
                        page++;

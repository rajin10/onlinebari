...
@section('content')
    ...
    @php
        $stotal = 0;
        $ids = [];
        $data = Cart::content()->sortBy('weight');
        foreach ($data as $item) {
            $whole = \App\Models\Product::find($item->id);
            if ($whole && !in_array("$whole->user_id", $ids)) {
                $ids[] = $whole->user_id;
            }
            if ($item->qty >= 6 && $whole && $whole->whole_price > 0) {
    ...
    <section id="premium-checkout" x-data="checkout()" x-cloak>
        <div class="pc-shell">
            ...
            <form action="{{ route('order.store_minimal') }}" method="POST" @submit.prevent="submitOrder($event)" novalidate>
                @csrf
                    ...
                    <div class="pc-col-form">
                        ...
                        <div class="pc-card">
                            ...
                            <div class="pc-field">
                                <input id="first_name" name="first_name" type="text" class="pc-input" placeholder=" "
                                    autocomplete="name" x-model="name"
                        ...
                        <div class="pc-card">
                            ...
                            <div class="pc-field" :class="{ 'pc-hidden': payment !== 'uddoktapay' }" id="email_wrap">
                                ...
                                <small class="pc-err" x-show="errors.email" x-text="errors.email"></small>
                            </div>
                        ...
                        <div class="pc-card">
                            ...
                            <div class="pc-pay-grid">
                                ...
                                @auth
                                    @if (setting('g_wallate') == 'true')
                                        <label class="pc-pay" :class="{ 'selected': payment === 'wallate' }">
                                            <input type="radio" name="payment_method" value="wallate" x-model="payment">
                                            <img src="{{ asset('/') }}icon/wallet.png" alt="Wallet" onerror="this.style.display='none'">
                                            <span>ওয়ালেট ({{ auth()->user()->wallate ?? 0 }})</span>
                                            <i class="pc-pay-check">✓</i>
                                        </label>
                                    @endif
                                @endauth
                            ...
                            {{-- Manual mobile-banking fields --}}
                            <template x-if="['Bkash','Nagad','Rocket'].includes(payment)">
                                <div class="pc-field-row">
                                    <div class="pc-field">
                                        <input type="text" name="mobile_number" class="pc-input" placeholder=" " required>
                                        <label>যে নম্বর থেকে পাঠিয়েছেন</label>
                                    </div>
                                    <div class="pc-field">
                                        <input type="text" name="transaction_id" class="pc-input" placeholder=" " required>
                    ...
                    <aside class="pc-col-summary">
                        <div class="pc-summary">
                            <button type="button" class="pc-summary-head" @click="summaryOpen = !summaryOpen">
                                <span>🧾 অর্ডার সারাংশ ({{ $data->count() }})</span>
                                <span class="pc-summary-total">
                                    <span id="total-mini">{{ number_format($stotal, 0) }}</span> {{ $currency }}
                            ...
                            <div class="pc-summary-body" x-show="summaryOpen" x-transition>
                                <div class="pc-items">
                                    @foreach ($data as $item)
                                        @php
                                            $w = \App\Models\Product::find($item->id);
                                            $line = $item->qty >= 6 && $w && $w->whole_price > 0 ? $item->qty * $w->whole_price : $item->subtotal;
                                        @endphp
                                        <div class="product pc-item">
                                            <img src="{{ asset('uploads/product/' . $item->options->image) }}" alt="{{ $item->name }}" onerror="this.style.visibility='hidden'">
                                            <div class="pc-item-info">
                                                <a href="{{ route('product.details', $item->options->slug) }}">{{ $item->name }}</a>
                                                <span class="pc-item-qty">পরিমাণ: {{ $item->qty }}</span>
                                            </div>
                                            <span class="pc-item-price" style="text-align: right">{{ number_format($line, 0) }} {{ $currency }}</span>
                                        </div>
                                    @endforeach
                                </div>
                                ...
                                <div class="pc-totals">
                                    <div class="pc-trow"><span>সাবটোটাল</span><span><span id="sub-total">{{ $stotal }}</span> {{ $currency }}</span></div>
                                    <div class="pc-trow"><span>শিপিং চার্জ</span><span>+ <span id="ship-charge">0.00</span> {{ $currency }}</span></div>
                                    ...
                                    <div class="pc-trow pc-grand"><span>সর্বমোট</span><span><span id="total">{{ $stotal }}</span> {{ $currency }}</span></div>
                                </div>
                            </div>
                        </div>
                    </aside>
...
@push('js')
        ...
        function checkout() {
            return {
                ...
                lookupCustomer() {
                    ...
                    fetch(`{{ route('checkout.lookup') }}?phone=${encodeURIComponent(this.phoneNormalized)}`, {
    ...
    {{-- Coupon + live total recompute (kept from existing logic) --}}
    <script>
        $(document).ready(function() {
            ...
            function recompute() {
                ...
                let seller_count = parseInt($('#seller_count').val()) || 0;
                let perCharge = parseFloat("{!! setting('shipping_charge_out_of_range') ?: 0 !!}") || 0;
                let freeAbove = parseFloat("{!! setting('shipping_free_above') ?: 0 !!}") || 0;
                let shipping = (freeAbove > 0 && stotal >= freeAbove) ? 0 : perCharge * seller_count;
                let coupon = parseFloat(($('span#coupon').text() || '0').replace(/,/g, '')) || 0;
                let total = Math.max(0, stotal + shipping - coupon);
                $('span#ship-charge').text(number_format(shipping, 2, '.', ','));
    ...
    {{-- GA4 begin_checkout + add_payment_info --}}
    <script>
        $(document).ready(function() {
            ...
            window.dataLayer.push({
                ...
                "ecommerce": {
                    ...
                    "items": [
                        @foreach ($data as $item)
                            { "item_id": "{{ $item->id }}", "item_name": @json($item->name), "price": {{ $item->price }}, "quantity": {{ $item->qty }} },
                        @endforeach
                    ]
                }
            });
            ...
            function checkPaymentInfo() {
                let name = $('#first_name').val(), phone = $('#phone').val();
                if (name && phone && !paymentInfoSent) {
                    window.dataLayer.push({ "event": "add_payment_info", "ecommerce": { "currency": "BDT", "value": {{ $stotal }} } });
    ...
    {{-- Incomplete-lead tracking (preserved) --}}
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let typingTimer;
            function payload() {
                return { name: nameInput.value.trim(), phone: phoneInput.value.trim(), email: emailInput?.value.trim() || '', address: addressInput?.value.trim() || '', page_type: 'cart_checkout' };
            }
            function save() {
                const d = payload();
                if (!d.name && !d.phone) return;
                fetch(leadUrl, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json' }, body: JSON.stringify(d) }).catch(() => {});
            }
            function onType() { clearTimeout(typingTimer); typingTimer = setTimeout(save, 1000); }
            nameInput.addEventListener('input', onType);
            phoneInput.addEventListener('input', onType);
            if (emailInput) emailInput.addEventListener('input', onType);
            if (addressInput) addressInput.addEventListener('change', save);
            window.addEventListener('beforeunload', function() {
                ...
                try { xhr.send(JSON.stringify(d)); } catch (e) {}
            });
        });
    </script>
@endpush

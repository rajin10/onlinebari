<div class="header-category-menu" style="float:left;">
    <ul class="mega">
        @foreach ($categories as $category)
            <li class="f-li">
                <a
                    href="{{ $category->sub_categories->count() ? 'javascript:void(0)' : route('category.product', $category->slug) }}">

                    <img style="width: 22px;height: 22px;margin-right: 10px;display: inline-block;"
                        src="{{ asset('uploads/category/' . $category->cover_photo) }}" alt="">
                    {{ $category->name }}
                </a>
                                ...
                                @if ($sub_category->miniCategory->count() > 0)
                                    ...
                                    @else
                                        <a style="padding:15px !important"
                                            href="{{ route('subCategory.product', $sub_category->slug) }}">
                                @endif
                                        ...
                                        @foreach ($sub_category->miniCategory as $miniCategory)
                                            <li style="display:inline-block !important">
                                                ...
                                                @if ($miniCategory->extraCategory->count() > 0)
                                                    ...
                                                    @else
                                                        <a
                                                            href="{{ route('miniCategory.product', $miniCategory->slug) }}">
                                                @endif

                                                <img style="width: 22px;height: 22px;margin-right: 10px;display: inline-block;"
                                                    src="{{ asset('uploads/mini-category/' . $miniCategory->cover_photo) }}"
                                                    alt="">
                                                {{ $miniCategory->name }}
                                                </a>
                                                @if ($miniCategory->extraCategory->count() > 0)
                                                    <ul class="sub-cat mcat">
                                                        <p class="cback"><span id="sub-close">{{ $category->name }}/
                                                                {{ $sub_category->name }}</span></p>
                                                        @foreach ($miniCategory->extraCategory as $extraCategory)
                                                            <li><a
                                                                    href="{{ route('extraCategory.product', $extraCategory->slug) }}">
                                                                    <img style="width: 22px;height: 22px;margin-right: 10px;display: inline-block;"
                                                                        src="{{ asset('uploads/extra-category/' . $extraCategory->cover_photo) }}"
                                                                        alt="">
...
<script>
    (function($, window) {
        $(document).ready(function() {
            ...
            $('ul').parent('li').prepend(
                '<div style="position: absolute;right: 0;top: 10px;color: #666;" class="aroow2 icon-right icofont icofont-simple-right"></div>'
            );

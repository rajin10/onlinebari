    ...
    <div class="col-md-4">
        <div class="thumbnail">
            <a href="{{ route('product.details', $product->slug) }}">
                <img style="width: 100%;height: 85px;object-fit: cover;"
                    src="{{ asset('uploads/product/' . $product->image) }}" alt="Product Image">
            </a>
        </div>
    </div>
        ...
        <p style="color: #333;font-size: 14px;">
            ...
            @if ($product->discount_price > 0)
                <span>
                    ...
                    <del>{{ setting('CURRENCY_CODE_MIN') ?? 'TK' }}.{{ $product->regular_price }}</del></span>
            @else
                <span><strong
                        style="color: var(--primary_color)">{{ setting('CURRENCY_CODE_MIN') ?? 'TK' }}.{{ $product->regular_price }}</strong></span>
            @endif
        </p>

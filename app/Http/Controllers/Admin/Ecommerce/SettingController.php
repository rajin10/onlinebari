...
class SettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($request->type == 1) {
            ...
            return back();
        } elseif ($request->type == 20) {
            // Google Sheets order logging
            Setting::updateOrCreate(['name' => 'GOOGLE_SHEETS_ENABLED'], ['value' => $request->get('GOOGLE_SHEETS_ENABLED', '0')]);
            Setting::updateOrCreate(['name' => 'GOOGLE_SHEETS_WEBHOOK_URL'], ['value' => trim((string) $request->get('GOOGLE_SHEETS_WEBHOOK_URL'))]);
            Setting::updateOrCreate(['name' => 'GOOGLE_SHEETS_SECRET'], ['value' => trim((string) $request->get('GOOGLE_SHEETS_SECRET'))]);

            notify()->success('Google Sheets settings updated', 'Success');
            ...
            Setting::updateOrCreate(['name' => 'BDCOURIER_STATUS'], ['value' => $request->get('BDCOURIER_STATUS')]);
            Setting::updateOrCreate(['name' => 'BDCOURIER_API_KEY'], ['value' => $request->get('BDCOURIER_API_KEY')]);

            notify()->success('Courier API settings successfully updated', 'Success');
    ...
    {
        return view('admin.e-commerce.setting.courierIndex');
    }

    public function googleSheetsIndex()
    {
        return view('admin.e-commerce.setting.googleSheetsIndex');
    }

    public function googleSheetsTest(GoogleSheetsService $sheets)
    {
        if (! $sheets->enabled()) {
            notify()->error('Webhook URL is empty or logging is turned off.', 'Not configured');

            return back();
        }

        if ($sheets->sendTest()) {
            notify()->success('Test row sent — check your Google Sheet.', 'Success');
        } else {
            notify()->error('Could not reach the webhook. Re-check the URL/secret and that the script is deployed to "Anyone".', 'Failed');
        }
